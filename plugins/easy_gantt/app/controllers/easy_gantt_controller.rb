class EasyGanttController < ApplicationController
	...
	def projects
		...
		@projects_issues_counts = Issue.gantt_opened.where(project_id: @projects).group(:project_id).count(:id)
	end

	def project_issues
		# TODO: Global route to skip rights
		@issues = Issue.gantt_opened.where(project_id: params[:project_id])
	...
	def load_projects
		...
		conditions = "#{p_table}.status = #{Project::STATUS_ACTIVE}" # default condition for projects(repeated in easy_gantt_issue_query.rb for issues line 103) 

		if @opened_project
			tree_conditions = @query.get_children_conditions(p_table, @opened_project)
			conditions = "#{conditions} AND (#{tree_conditions})" # we have to add parenthesis since tree_condition is a list
		end

		# we want all levels of children
		@projects.concat Project.where(conditions)

		# this was only showing the opened project folder and hiding subprojects
