...
ysy.data.extender(ysy.data.Data, ysy.data.Issue, {
  ...
  isIssue: true,
  _postInit: function () {
    if (this.start_date && !this.relative_start) {
      this.relative_start = false;
    ...
    if (this.end_date && !this.relative_end) {
      this.relative_end = false;
      this.end_date._isEndDate = true;
      this._end_date = this.end_date;
    } else {
      this.relative_end = true;
  ...
  getDuration: function (unit) {
    ...
    var start = moment(this.relative_start? this.relative_start_date: this._start_date);
    var durationPack = gantt._working_time_helper.get_work_units_between(start, this._end_date, "all");
    if (!this._cache) {
      this._cache = {};
    }
  ...
  pushSelf: function (days, visited) {
    ...
    ysy.history.openBrack();
    var res = true;
    if (/*days > 0*/false) { // days is negative when a following task has a delay, and since we want the same behavior for negative delay, this is now not necessary nor wanted
      var duration = this.getDuration();
      //gantt._working_time_helper.round_date(target.start_date.);
      var start_date =  moment(this.relative_start? this.relative_start_date: this._start_date);
      ...
      if(!this.relative_end)
        this.set({end_date: end_date});
      if(!this.relative_start)
...
ysy.data.extender(ysy.data.Data, ysy.data.Relation, {
  ...
  getSourceDate: function (source) {
    ...
    if (this.type === "precedes") return source.relative_end? source.relative_end_date: source.end_date;
  ...
  getTargetDate: function (target) {
    ...
    if (this.type === "precedes") return target.relative_start? target.relative_start_date: target.start_date;
  ...
  checkDelay: function () {
    var del = this.getActDelay();
    return true;//del >= (this.delay || 0); // causes links to get in red without error output
  },
  getSource: function () {
