...
ysy.view.addGanttAddons = function () {
  ...
  gantt.moveDesc = function (task, diff, ancestorLink, resizing) {
    ...
    if(task.relative_end)// remove 1 day, 1 time, on the task item
     end_date = moment(end_date) - 86400; //the relative end dont count the day in which it ends (at midnight) so to counter effect the later correction of +1 (line 223) we need to remove one day here (86400 seconds)
    for (var i = 0; i < task.$source.length; i++) {
      var lid = task.$source[i];
      var link = gantt._lpull[lid];
      if(link.skipPush) continue;
      var desc = gantt._pull[link.target];
      if(!desc.relative_start || !desc.relative_end) continue;
      if (diff < 0 && bothDir) {
  ...
  gantt.refreshDesc = function(seed_task){
      if(!seed_task)
        return;
      var task = gantt.getTask(seed_task);
      task.widget.repaintRequested = true;
      for (var i = 0; i < task.relation_source_id.length; i++)
      {
          var relation = ysy.data.relations.getByID(task.relation_source_id[i]);
          gantt.refreshDesc(relation.target_id);
      }
  };
  ...
  gantt.getFreeDelay = function (link, desc, ascStartDate, ascEndDate) {
    ...
    var delayRatio = targetDate / 1000 - sourceDate;
    if(delayRatio > 86400 && gantt._working_time_helper.is_weekend(moment(targetDate).subtract(12, 'hours'))) // 86400 is 1 day in seconds
        correction = 3;// when there is a weekend betwen 2 related tasks the following one need to move before the preceding one reach it over in the weekend and correct itself afterward
    var delay = delayRatio / daysToSeconds;
