...
gantt._tasks_dnd = {
  ...
  _resize : function(ev, shift, drag){
    ...
    var issue = ysy.data.issues.getByID(ev.id);
    ...
    if(drag.left){
      ...
      //gantt.ascStop(ev, start_date); // we can extend pass the end of the preceding task.
      if(issue.relative_start)
      {
          var working_days = gantt._working_time_helper.get_work_units_between(start_date, ev.end_date, "day").toFixed(0);
          ev.estimated = working_days * 8;
      }
      ...
      if(issue.relative_end || issue.relative_start)
      {
          var real_end_date = moment(gantt.dateFromPos(coords_x.end + shift.x));
          var working_days = gantt._working_time_helper.get_work_units_between(start_date, real_end_date, "day").toFixed(0);
          ev.estimated = working_days * 8;
      }
      end_date._isEndDate = true;
      gantt.ascStop(ev, undefined, end_date);
  ...
  _move : function(ev, shift, drag){
    ...
    var issue = ysy.data.issues.getByID(ev.id);
    var coords_x = this._drag_task_coords(ev, drag);
    var new_start = moment(gantt.dateFromPos(coords_x.start + shift.x));
    var new_end = moment(gantt.dateFromPos(coords_x.end + shift.x)).subtract(1, "days");
    new_end._isEndDate = true; // HOSEK < A V
    if (!new_start.isValid() || !new_end.isValid()) {
      debugger;
    }
    //gantt.ascStop(ev, new_start, new_end); // commented this call since we want to be able to move the relative issues at minus delays.
    var old_start = ev.start_date;
    if(!new_start){
      ev.start_date = gantt.date.Date(gantt.getState().min_date);
      ...
      ev.start_date = issue.relative_start && !issue.relative_end? gantt.getRelativeDate(new_end, issue.estimated_hours - 8, true): new_start; // removing time since the end day will be counted as a working day since it's an absolute end
      ev.end_date = issue.relative_end ? gantt.getRelativeDate(new_start, issue.estimated_hours, false): new_end;
    }
    var diff = ev.start_date.diff(old_start, "seconds");
    if (ev.type === "project") {
      ev.maximal_start = null;
      ev.minimal_end = null;
    } else {
      gantt.moveDesc(ev, diff);
    }
  ...
  on_mouse_up : function(e){
    ...
    if (drag.mode && drag.id) {
      //drop
      var ev = gantt.getTask(drag.id);
      if(ev.relative_start && !ev.relation_target_id && drag.mode == 'move')
        ev.relative_start = false;
      //if(gantt.config.work_time && gantt.config.correct_work_time){
...
gantt.getRelativeDate = function(date, duration, isEnd){
    var working_hours = 0;
    var non_working_hours = 0;
    var real_date = moment(this._working_time_helper.get_closest_worktime({date: moment(date), dir: isEnd? "past": "future", unit:"day"}));
    var estimated_extent = isEnd? moment(real_date).subtract(duration/8, 'd'): moment(real_date).add(duration/8, 'd');
    var iter = 0;
    while(Math.abs(working_hours) < duration)
    {
        working_hours = this._working_time_helper.get_work_units_between(real_date, estimated_extent, "hour");
        non_working_hours = -working_hours + (isEnd? -duration : duration);
        estimated_extent = moment(estimated_extent).add((non_working_hours)/8, 'd');
        iter ++;
        if(iter > 3000) // same number they are using for breaking possible infinite loop
            break;
    }
