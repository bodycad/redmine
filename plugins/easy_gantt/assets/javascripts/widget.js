...
ysy.view.extender(ysy.view.Widget, ysy.view.TaskTooltip, {
  ...
  out: function () {
    ...
    return {
      name: model.name,
      estimated: model.estimated || model.estimated_hours,
      start_date: model.relative_start ? model.relative_start_date.format(gantt.config.date_format): (model.start_date? model.start_date.format(gantt.config.date_format): moment(null)),
      end_date: model.relative_end ? model.relative_end_date.format(gantt.config.date_format): (model.end_date? model.end_date.format(gantt.config.date_format): moment(null)),
      relative_start: model.relative_start,
      relative_end: model.relative_end,
      columns: columns,
...
ysy.view.extender(ysy.view.Widget, ysy.view.LinkPopup, {
  ...
  requestRepaint: function () {
    gantt.refreshDesc(this.model.target_id); // the widget is not refreshed by default when the link delay is changed. so it's manually done here.
